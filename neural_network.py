...
class NeuralNet:
    ...
    def compute_activations(self, x, target):
        ...
        for layer in self._layers:
            next_output = layer.compute_activation(next_output)

        output = next_output # Output of the last layer
        loss = self.loss.compute_activation(next_output, target) # Compute the loss
    ...
    def compute_gradients(self):
        ...
        grads = self.loss._input_error_gradient
        for layer in reversed(self._layers):
            layer._output_error_gradient = grads
            layer.compute_gradient()
            grads = layer._input_error_gradient
