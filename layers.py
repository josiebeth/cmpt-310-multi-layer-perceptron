...
class DenseLayer(BaseLayer):
    ...
    def compute_activation(self, x):
        ...
        output = np.add(np.dot(x, self.w), self.b)
    ...
    def compute_gradient(self):
        ...
        de_do = self._output_error_gradient
        x = self._input_data
        w = self.w

        self.dw = np.dot(np.transpose(x), de_do)
        self.db = np.dot(np.ones((1, de_do.shape[0])), de_do)
        self._input_error_gradient = np.dot(de_do, np.transpose(w))
    ...
    def update_weights(self, learning_rate):
        ...
        self.w = np.subtract(self.w, np.dot(learning_rate, self.dw))
        self.b = np.subtract(self.b, np.dot(learning_rate, self.db))
